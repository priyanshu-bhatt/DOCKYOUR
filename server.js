...
app.get("/login",(req,res) => {
    ...
    exec('docker login -u '+user_name+' '+ ' -p '+ pass_word,(err,stdout,stderr) => {
...
const docker_file_content=req.query.dfile;
        ...
        exec("cat dockerfile && echo '<br /><b>sucessfully created Dockerfile</b>' ",(err,stdout,stderr) => {
                        fs.writeFile('/root/DOCYOUR/dockerfile', docker_file_content.split(";").toString().replaceAll(',', '\n'), err => {
                                        if (err) {
                                        console.error(err);
                                        }})
...
app.listen(3001,() => { console.log("container ap started")})
